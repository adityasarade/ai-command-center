...
export default function Page() {
  return (
    <>
      ...
      <CodeBlock lang="text" label="precedence" code={`defaults
  ...
  "branding": { "name": "AI Command Center", "accent": "#3987e5", "tagline": "..." }
      ...
      <table>
        <thead><tr><th>Var</th><th>Purpose</th></tr></thead>
        <tbody>
          <tr><td><code>AICC_PORT</code></td><td>Gateway port</td></tr>
          <tr><td><code>AICC_HOST</code></td><td>Bind host</td></tr>
          <tr><td><code>AICC_DATA_DIR</code></td><td>Telemetry directory</td></tr>
          <tr><td><code>AICC_PRESET</code></td><td>Config preset name</td></tr>
          <tr><td><code>AICC_GATEWAY</code></td><td>Gateway origin (used by the SDKs/examples)</td></tr>
          <tr><td><code>AICC_KEY</code></td><td>Project gateway key (used by the SDKs when auth is on)</td></tr>
        </tbody>
      </table>
...
# or in a config file:  { "preset": "medikabazaar", "keys": { ... } }`} />
      ...
      <p>
        Costs are stored in USD and converted for display. Rates are refreshed every 12 hours (ECB via
        frankfurter.app, with an open.er-api.com fallback and a built-in offline fallback) and cached
        in your data dir. Pin them yourself to skip fetching entirely:
      </p>
      <CodeBlock lang="jsonc" code={`"currency": { "default": "INR", "options": ["INR","USD","EUR"], "rates": { "INR": 84, "EUR": 0.92 } }`} />
