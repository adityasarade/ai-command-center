...
export default function Page() {
  return (
    <>
      <h1>HTTP API</h1>
      <p className="lead">
        Everything the dashboard shows is a plain JSON endpoint you can build on. When auth is locked,
        the <code>/api/*</code> read endpoints require a session cookie; <code>/api/track</code> also
        accepts a project gateway key.
      </p>
      ...
      <table>
        <thead><tr><th>Method + path</th><th>Returns</th></tr></thead>
        <tbody>
          <tr><td><code>GET /health</code></td><td><code>{'{ ok, name, version, uptimeMs }'}</code></td></tr>
          <tr><td><code>GET /api/meta</code></td><td>version, branding, currency, providers, record count</td></tr>
          <tr><td><code>GET /api/stats?range=7d&amp;project=x</code></td><td>totals, timeseries, by-project / by-model / by-provider</td></tr>
      ...
      <p><code>range</code> accepts <code>1h · 24h · 7d · 30d · 90d · all</code>, or explicit <code>from</code>/<code>to</code> epoch-ms.</p>

      <h2>POST /api/track</h2>
      <p>Report usage the proxy can&apos;t see. Cost is computed for you if you omit <code>costUsd</code>. Send one object or an array.</p>
      <CodeBlock lang="bash" code={`curl -X POST http://localhost:4321/api/track \\
  ...
  -d '{
    ...
    "latencyMs": 820
      ...
      <CodeBlock lang="bash" code={`curl -s "http://localhost:4321/api/stats?range=24h" \\
