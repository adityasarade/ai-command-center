  ...
  return (
    <>
      ...
      <CodeBlock
        lang="text"
        label="flow"
        code={`your app  ──base_url──▶  AI Command Center gateway  ──▶  OpenAI / Anthropic / Gemini / …
                              │
      ...
      <p>
        <strong>It is:</strong> a self-hosted, language-agnostic cost/usage dashboard that runs from
        a single command with no database and no external services. Your provider keys pass through;
        prompt and response bodies are never stored.
      </p>
      <p>
        <strong>It isn't:</strong> a tracing platform, an eval framework, a prompt manager, or a
        routing/failover gateway. If you need those, the{' '}
        <Link href="/docs/comparison">comparison</Link> points you to the right tool.
      </p>

      <h2>Next steps</h2>
      <ul>
        <li>
          <Link href="/docs/install">Install &amp; run</Link> - get the gateway up in under a
          minute.
        </li>
        <li>
          <Link href="/docs/integrate">Integrate a project</Link> - snippets for every language.
        </li>
        <li>
          <Link href="/docs/auth">Auth &amp; teams</Link> - lock it down for shared use.
        </li>
        <li>
          <Link href="/docs/api">HTTP API</Link> - build on the raw JSON endpoints.
        </li>
      </ul>
      ...
      <div className="callout">
        ...
        <a href="https://github.com/adityasarade/ai-command-center/blob/main/AGENTS.md">
          AGENTS.md
        </a>
        .
      </div>
