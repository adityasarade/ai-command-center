...
client = OpenAI(base_url="http://localhost:4321/p/invoice-bot/openai/v1")` },
  { label: 'JS / TS', lang: 'js', code: `import OpenAI from "openai";
const client = new OpenAI({ baseURL: "http://localhost:4321/p/invoice-bot/openai/v1" });` },
  { label: 'Java', lang: 'java', code: `OpenAIClient client = OpenAIOkHttpClient.builder()
    .fromEnv()
    .baseUrl("http://localhost:4321/p/invoice-bot/openai/v1")
    .build();` },
  { label: 'env var', lang: 'bash', code: `export OPENAI_BASE_URL="http://localhost:4321/p/invoice-bot/openai/v1"` },
  { label: 'LangChain', lang: 'python', code: `from langchain_openai import ChatOpenAI
  ...
  return (
    <>
      ...
      <p className="lead">
        ...
        changes. The <code>/p/&lt;project&gt;</code> segment is how calls are grouped on the dashboard.
      </p>

      <h2>The pattern</h2>
      <p>Every gateway route looks like one of these:</p>
      <CodeBlock lang="text" label="routes" code={`http://localhost:4321/p/<project>/openai/v1        →  api.openai.com
...
client = Anthropic(base_url="http://localhost:4321/p/invoice-bot/anthropic")`} />
      ...
      <h2>Google Gemini</h2>
      <CodeBlock lang="python" code={`from google import genai
...
init({ project: "support-bot" });`} />
      ...
      <p>Can&apos;t route through the proxy? Report usage directly and it&apos;s priced and dashboarded the same way:</p>
  ...
  -d '{"project":"nightly-job","provider":"openai","model":"gpt-4o","tokensIn":52000,"tokensOut":9000}'`} />
      ...
      <p>If you can&apos;t change the path, send a header instead: <code>x-aicc-project: invoice-bot</code>.</p>
